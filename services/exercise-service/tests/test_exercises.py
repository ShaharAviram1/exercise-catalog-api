...
def test_create_valid_exercise_successfully():
    with exercise_client() as client:
...
def test_reject_invalid_muscle_group():
    with exercise_client() as client:
        response = client.post("/exercises", json=exercise_payload(primary_muscles=["wings"]))
...
def test_reject_invalid_media_url():
    with exercise_client() as client:
        response = client.post("/exercises", json=exercise_payload(media_url="not-a-valid-url"))
...
def test_update_non_existent_exercise():
    with exercise_client() as client:
        response = client.put("/exercises/999", json=exercise_payload(name="Updated Push Up"))
